...
function eliminarDashboard() {
  ...
  fetch(`${API_BASE}dashboard/delete`, {
    method: 'POST',
    body: formData
  })
  .then(res => res.json())
  .then(data => {
    if (data.success) {
      alert(data.message);
      window.location.href = '/Hackdash-aiweekend/frontend/dashboard';
    } else {
...
fetch(`${API_BASE}dashboard/get?slug=${slug}`)
  .then(res => {
...
function viewProject(projectId) {
  ...
  window.location.href = `/Hackdash-aiweekend/frontend/project?id=${projectId}&dashboardSlug=${dashboardSlug}`;
}
...
function loadProjectProgress(projectId) {
  fetch(`${API_BASE}project/stats?id=${projectId}`)
    .then(response => response.json())
    .then(data => {
      if (data.success) {
