...
function renderProjectDetails(project) {
    ...
    function createFormData(projectId, userData) {
        ...
        Object.entries(userData).forEach(([key, value]) => form.append(key, value));
        return form;
    }
    ...
    function setupProjectButton(button, action, oppositeButton, apiEndpoint) {
        ...
        button.addEventListener('click', async () => {
            ...
            if (!userEmail || (action === 'join' && !userName)) {
                joinStatus.textContent = `Inicia sesión para ${action === 'join' ? 'unirte' : 'abandonar'}.`;
                return;
            }
            ...
            try {
                ...
                // Make API request
                const response = await fetchWithTimeout(`${API_BASE}project/${action}`, {
                    method: 'POST',
                    body: formData
                });
